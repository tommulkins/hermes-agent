...
def test_run_stdio_malware_check_times_out_fail_open():
    ...
    def hung_check(_command, _args):
        time.sleep(2)  # outlasts the 0.2s timeout; bounded so it can't stall teardown
    ...
    async def _test():
        with patch("tools.osv_check.check_package_for_malware", side_effect=hung_check), \
            ...
            await server.shutdown()
        # Returned shortly after the 0.2s timeout (fail-open), not the 30s hang.
        assert elapsed < 3.0, f"startup did not fail-open promptly ({elapsed:.1f}s)"
